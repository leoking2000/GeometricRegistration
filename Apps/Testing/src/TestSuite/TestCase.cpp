...
namespace tests
{
	...
	TestResult RunEfficientICPPointToPlane(const TestCase& test, const geo::EfficientICPParams& params)
	{
		...
		geo::EfficientICPResult result = 
			geo::EfficientICP(test.target->cloud, test.source->cloud, test.source->cloud,
	...
	void LogTestResult(const TestResult& result)
	{
		...
		std::cout << "    RMSE:       " << std::setprecision(6) << result.icp_result.rmse << "\n";
		...
		geo::Distance(result.icp_result.transform, result.testCase.groundTruth, rotErr, transErr);
		...
		std::cout << "    Rotation:    " << std::setprecision(3) << rotErrDeg << " deg"
			<< "  [" << RotationLabel(rotErrDeg) << "]\n";

		if (hasMesh)
		{
			std::cout << "    Translation: " << std::setprecision(4) << transErr
				<< "  (" << std::setprecision(2) << transErrPct << "% of diagonal)"
